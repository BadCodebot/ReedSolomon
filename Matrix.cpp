	...
	Vector operator*(const Vector& v, const Matrix& m) {
		...
		for (int column = 0; column < m.GetColumns(); column++) {
			for (int row = 0; row < m.GetRows(); row++) {
				rv[column] = GaloisField16::Add(rv[column], GaloisField16::Multiply(v[row], m[row][column]));
			}
		}
		return rv;
	}
	...
	Matrix operator*(const Matrix& a, const Matrix& b) {
		...
		for (int row = 0; row < a.GetRows(); row++) {
			for (int col = 0; col < b.GetColumns(); col++) {
				for (int i = 0; i < a.GetColumns(); i++) rv[row][col] = GaloisField16::Add(rv[row][col], GaloisField16::Multiply(a[row][i], b[i][col]));
			}
		}
		return rv;
	}
