...
namespace ReedSolomon {
	...
	Vector& Vector::operator+=(const Vector& v) {
		for (int i = 0; i < length; i++) elements[i] = GaloisField16::Add(elements[i], v.elements[i]);
		return *this;
	}

	Vector& Vector::operator*=(uint16_t a) {
		for (int i = 0; i < length; i++) elements[i] = GaloisField16::Multiply(a, elements[i]);
		return *this;
	}
	...
	uint16_t InnerProduct(const Vector& a, const Vector& b) {
		uint16_t rv = 0;
		for (int i = 0; i < a.GetLength(); ++i) GaloisField16::Add(rv, GaloisField16::Multiply(a[i], b[i]));
		return rv;
	}
